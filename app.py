...
# FastAPI App Setup
# ============================================================
app = FastAPI(title="DROGON BIND TOOL", version="3.0")
...
async def api_bind_info(access_token: str = Form(...)):
    result = {"success": False, "player": {}, "bind": {}, "error": None}
    try:
...
async def api_bind_send_otp(access_token: str = Form(...), email: str = Form(...)):
    ...
    try:
        ...
        return JSONResponse(format_response_result(resp.text))
    except Exception as e:
        return JSONResponse({"status": "error", "message": str(e)})

@app.post("/api/bind-email/verify-otp")
async def api_bind_verify_otp(access_token: str = Form(...), email: str = Form(...), otp: str = Form(...)):
    headers = {"User-Agent": "GarenaMSDK/4.0.30", "Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json"}
...
async def api_bind_finalize(access_token: str = Form(...), email: str = Form(...), verifier_token: str = Form(...), security_code: str = Form(...)):
    headers = {"User-Agent": "GarenaMSDK/4.0.30", "Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json"}
    bind_url = "https://100067.connect.garena.com/game/account_security/bind:create_bind_request"
    bind_data = {"email": email, "app_id": "100067", "access_token": access_token, "verifier_token": verifier_token, "secondary_password": security_code}
    try:
...
async def api_unbind_verify_identity_otp(access_token: str = Form(...), email: str = Form(...), otp: str = Form(...)):
    headers = {"User-Agent": "GarenaMSDK/4.0.30", "Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json"}
...
async def api_unbind_verify_identity_code(access_token: str = Form(...), email: str = Form(...), security_code: str = Form(...)):
    headers = {"User-Agent": "GarenaMSDK/4.0.30", "Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json"}
...
async def api_unbind_finalize(access_token: str = Form(...), identity_token: str = Form(...)):
    headers = {"User-Agent": "GarenaMSDK/4.0.30", "Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json"}
...
async def api_change_verify_identity_otp(access_token: str = Form(...), email: str = Form(...), otp: str = Form(...)):
    headers = {"User-Agent": "GarenaMSDK/4.0.30", "Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json"}
    verify_url = "https://100067.connect.garena.com/game/account_security/bind:verify_identity"
    verify_data = {"email": email, "app_id": "100067", "access_token": access_token, "otp": otp}
    try:
...
async def api_change_verify_identity_code(access_token: str = Form(...), email: str = Form(...), security_code: str = Form(...)):
    headers = {"User-Agent": "GarenaMSDK/4.0.30", "Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json"}
    hashed_sec_code = hashlib.sha256(security_code.encode('utf-8')).hexdigest()
    verify_url = "https://100067.connect.garena.com/game/account_security/bind:verify_identity"
    verify_data = {"email": email, "app_id": "100067", "access_token": access_token, "secondary_password": hashed_sec_code}
...
async def api_change_send_otp_new(access_token: str = Form(...), new_email: str = Form(...)):
    headers = {"User-Agent": "GarenaMSDK/4.0.30", "Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json"}
    send_otp_url = "https://100067.connect.garena.com/game/account_security/bind:send_otp"
    send_otp_data = {"email": new_email, "locale": "en_PK", "region": "PK", "app_id": "100067", "access_token": access_token}
    try:
        resp = requests.post(send_otp_url, headers=headers, data=send_otp_data, timeout=15)
        return JSONResponse(format_response_result(resp.text))
    except Exception as e:
...
async def api_change_verify_otp_new(access_token: str = Form(...), new_email: str = Form(...), otp: str = Form(...)):
    headers = {"User-Agent": "GarenaMSDK/4.0.30", "Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json"}
    verify_url = "https://100067.connect.garena.com/game/account_security/bind:verify_otp"
    verify_data = {"email": new_email, "app_id": "100067", "access_token": access_token, "otp": otp}
    try:
        resp = requests.post(verify_url, headers=headers, data=verify_data, timeout=15)
        result = format_response_result(resp.text)
        try:
...
async def api_change_finalize(access_token: str = Form(...), identity_token: str = Form(...), new_email: str = Form(...), verifier_token: str = Form(...)):
    headers = {"User-Agent": "GarenaMSDK/4.0.30", "Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json"}
...
async def api_cancel_bind(access_token: str = Form(...)):
    ...
    cancel_data = {"app_id": "100067", "access_token": access_token}
    try:
...
async def api_eat_to_token(eat_input: str = Form(...)):
    result = {"success": False, "access_token": None, "player": {}, "error": None}
    try:
        eat_token = None
...
async def api_revoke_token(access_token: str = Form(...)):
    result = {"success": False, "player": {}, "error": None}
    try:
        ...
        result["player"] = {"nickname": nickname, "account_id": account_id, "region": region}
...
async def api_login_history(token: str = Form(...)):
    result = {"success": False, "player": {}, "history": [], "error": None}
    try:
        jwt_token = None
        open_id = None
...
async def api_bound_accounts(access_token: str = Form(...)):
    result = {"success": False, "bounded_accounts": [], "available_platforms": [], "error": None}
    try:
        ...
        for p_id in bounded_accounts:
            result["bounded_accounts"].append({"id": p_id, "name": PLATFORM_MAP.get(p_id, f"Unknown ({p_id})")})
        for p_id in available_platforms:
            result["available_platforms"].append({"id": p_id, "name": PLATFORM_MAP.get(p_id, f"Unknown ({p_id})")})
        result["success"] = True
    except Exception as e:
        result["error"] = str(e)
    return JSONResponse(result)
...
async def api_token_to_jwt(access_token: str = Form(...)):
    result = {"success": False, "jwt_token": None, "player": {}, "credits": {}, "error": None}
    try:
...
async def api_ban_account(access_token: str = Form(...)):
    ...
    try:
        ...
        result["response"] = {"status_code": response.status_code, "text": response.text}
        if response.status_code == 200:
            try:
...
async def api_owner():
    return JSONResponse({
        "developer": "SKY DROGON",
        "channel": "https://t.me/XERAS_DE",
        "version": "v3.0 (Premium Dragon Edition)",
